...
Версия: 2026-03-30-7
Изменения: фикс индикатора режима, кнопки удаления доменов, рестарт с обновлением whitelist
"""
...
def has_whitelist_changes() -> bool:
    if not WHITELIST_FILE.exists(): return True
    sys_wl = ZAPRET_DIR / "ipset" / "zapret-hosts-user.txt"
    if not sys_wl.exists(): return True
    return set(WHITELIST_FILE.read_text(encoding="utf-8").splitlines()) != set(sys_wl.read_text(encoding="utf-8").splitlines())
...
def get_zapret_status() -> tuple:
    ...
    try:
        ...
        mode = "unknown"
        if active:
            # Используем systemctl status вместо ps aux
...
class ManagementTab(QWidget):
    ...
    def init_ui(self):
        ...
        # Кнопки управления (отдельная строка, на всю ширину)
        btn_row = QHBoxLayout()
    ...
    def refresh_domains(self):
        ...
        for domain in domains:
            # Создаём контейнер для строки домена
            row = QHBoxLayout()
...
class ZapretManager(QMainWindow):
    ...
    def on_tab_changed(self, index):
        if index == 1:
            self.management_tab.refresh_domains()
    ...
    def apply_settings(self, pwd: str) -> bool:
        ...
        if selected_strategy:
            ...
            # Извлекаем параметры из стратегии (убираем nfqws/tpws префикс)
            strategy_params = selected_strategy.replace("nfqws ", "").replace("tpws ", "").strip()
            # Записываем стратегию в конец конфига
            strategy_line = f'NFQWS_OPT="--filter-tcp=80 {strategy_params} --new\n--filter-tcp=443 {strategy_params} --new\n--filter-udp=443 {strategy_params}"'
            ok, out, err = run_sudo(["sudo", "bash", "-c", f"echo \"{strategy_line}\" >> {CONFIG_FILE}"], pwd)
            if not ok:
                print(f"⚠️ Не удалось применить стратегию: {err}")
