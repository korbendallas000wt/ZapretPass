...
Версия: 2026-03-30-6
Изменения: фикс конфигов (MODE_FILTER), интерфейс вкладки 2 (резиновые скроллы, кнопки на всю ширину)
"""
...
def init_config_templates():
    """Создаёт шаблоны конфигов с правильным MODE_FILTER"""
    if not CONFIG_FILE.exists():
        return
    
    # Создаём backup если нет
    ...
    # === Whitelist режим (MODE_FILTER=hostlist) ===
...
def get_zapret_status() -> tuple:
    ...
    try:
        ...
        mode = "unknown"
        if active:
            r2 = subprocess.run("ps aux | grep -E 'nfqws|tpws' | grep -v grep", shell=True, capture_output=True, text=True)
...
class ManagementTab(QWidget):
    ...
    def refresh_domains(self):
        ...
        for domain in domains:
            ...
            self.domain_layout.insertWidget(self.domain_layout.count()-1, cb)
