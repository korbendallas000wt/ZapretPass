...
Фикс: отправка Enter после blockcheck + упрощённый парсер по маркерам
"""
...
class BlockcheckThread(QThread):
    ...
    def run(self):
        try:
            ...
            # 🔥 КЛЮЧЕВОЙ ФИКС: отправляем финальный Enter для "press enter to continue"
            try:
                time.sleep(0.5)
...
class SiteAccessibleDialog(QDialog):
    def __init__(self, domain: str, details: str, parent=None):
        super().__init__(parent)
        self.setWindowTitle("🟢 Сайт доступен без обхода")
        self.setModal(True)
        self.force_check = False
        l = QVBoxLayout(self)
        l.addWidget(QLabel(f"<b>{domain}</b>"))
        l.addWidget(QLabel(f"Статус: <b>{details}</b>"))
        l.addWidget(QLabel("Всё равно искать стратегию?"))
        bl = QHBoxLayout()
        by = QPushButton("✅ Да, искать"); by.clicked.connect(self.accept)
        bn = QPushButton("❌ Нет, пропустить"); bn.clicked.connect(self.reject)
        bf = QPushButton("⚡ Всё равно проверить")
        bf.setStyleSheet("QPushButton{background:#f39c12;color:white;font-weight:bold}")
        bf.clicked.connect(self.force_check_clicked)
        bl.addWidget(by); bl.addWidget(bn); bl.addWidget(bf); l.addLayout(bl)
    def force_check_clicked(self): self.force_check = True; self.accept()
...
class BlockcheckTab(QWidget):
    ...
    def init_ui(self):
        ...
        sl.addRow("IP:",self.cb_ip); sl.addRow("HTTP:",self.cb_http)
        sl.addRow("TLS1.2:",self.cb_t2); sl.addRow("TLS1.3:",self.cb_t3)
        sl.addRow("QUIC:",self.cb_qu); sl.addRow("Повторы:",self.sb_rep); sl.addRow("Режим:",self.cb_mod)
    ...
    def check_and_ask(self):
        ...
        if accessible:
            dialog = SiteAccessibleDialog(d, details, self)
            result = dialog.exec_()
            if result == QDialog.Rejected:
                self.status.setText(f"⚪ {d} - пропущен (доступен)")
                return
            if dialog.force_check:
                self.cb_mod.setCurrentText("3")
                self.status.setText(f"⚡ {d} - принудительный тест (force mode)")
            else:
                self.status.setText(f"🔧 {d} - поиск стратегии...")
        else:
            self.status.setText(f"⚠️ {d} - {details}")
            reply = QMessageBox.question(self, "⚠️ Сайт недоступен",
                f"Сайт {d} не отвечает корректно:\n{details}\n\nВсё равно запустить blockcheck?",
