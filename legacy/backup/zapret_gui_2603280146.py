...
Парсер стратегий: сплит по ' : ', поддержка nfqws/tpws, дедубликация
"""
...
def parse_strategies_from_output(output: str) -> list:
    """
    Парсит стратегии из блока * SUMMARY через сплит по ' : '
    - Поддерживает nfqws и tpws
    - Игнорирует 'working without bypass'
    - Убирает полные дубликаты
    """
    strategies, in_summary = [], False
    
    for line in output.split('\n'):
        # Ищем начало блока SUMMARY
        if '* SUMMARY' in line:
            in_summary = True
            continue
        
        # Конец блока - служебные сообщения или пустая строка после начала
        ...
        # Игнорируем "working without bypass" - это не стратегия для сохранения
        if 'working without bypass' in right_part.lower():
            continue
        
        # Проверяем, начинается ли с nfqws или tpws
        if right_part.startswith(('nfqws ', 'tpws ')):
            # Сохраняем всю строку как есть (с именем утилиты)
            if right_part not in strategies:
                strategies.append(right_part)
