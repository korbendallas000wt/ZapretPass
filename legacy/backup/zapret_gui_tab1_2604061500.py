...
class BlockcheckTab(QWidget):
    ...
    def save_strategies_action(self):
        d = self.get_domain()
        if not d or not self.current_strategies: return
        success, msg, old_count = save_strategies(d, self.current_strategies, self.current_settings)
        if not success: QMessageBox.critical(self,"❌", msg); return
        if old_count > 0:
            dialog = OverwriteDialog(d, old_count, len(self.current_strategies), self)
            if dialog.exec_() == QDialog.Rejected:
                try: (STRATEGIES_DIR / f"{d}.json").unlink()
                except: pass
                self.status.setText("💾 Отменено"); return
