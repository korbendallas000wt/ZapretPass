...
Исправления: одна кнопка (запустить/остановить), статус без рамки
"""
...
BC_DEFAULTS = {"ipver":"4","http":"Y","tls12":"Y","tls13":"N","quic":"N","repeat":1,"mode":"1"}
...
def init_whitelist():
    """Копирует системный whitelist в пользовательский при первом запуске"""
    ...
    if not WHITELIST_FILE.exists() and SYS_WHITELIST.exists():
        try:
            ...
            print(f"📋 Инициализирован пользовательский whitelist из {SYS_WHITELIST}")
        except Exception as e:
            print(f"⚠️ Не удалось инициализировать whitelist: {e}")
    elif not WHITELIST_FILE.exists():
        WHITELIST_FILE.write_text("", encoding="utf-8")
        print(f"📋 Создан пустой пользовательский whitelist")
...
def cache_sudo(password: str) -> bool:
    try:
        proc = subprocess.Popen(["sudo","-S","-v"], stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, text=True)
...
def run_sudo(cmd: list, password: str = None) -> tuple:
    try:
        if password: cache_sudo(password)
...
class BlockcheckThread(QThread):
    ...
    def __init__(self, domain: str, settings: dict, password: str):
        super().__init__()
        self.domain = domain
    ...
    def run(self):
        try:
            ...
            cache_sudo(self.password)
            cmd = ["sudo", "-S", "bash", "-c", f"cd {ZAPRET_DIR} && ./blockcheck.sh {self.domain}"]
            self.output_ready.emit(f"🔍 Запуск blockcheck для {self.domain}...\n\n")
            ...
            self.finished.emit(success, strategies, self.settings)
...
def parse_strategies_from_output(output: str) -> list:
    ...
    for line in output.split('\n'):
        if '* SUMMARY' in line: in_summary = True; continue
        if 'press enter to continue' in line.lower(): break
        if in_summary and 'not working' not in line.lower():
            match = re.search(r':\s*((?:nfqws|tpws)\s+--[^!\n]+)', line)
            if match:
                p = match.group(1).strip()
                if p and p not in strategies: strategies.append(p)
    return strategies
...
class BlockcheckTab(QWidget):
    ...
    def init_ui(self):
        ...
        # Кнопки действий — ОДНА КНОПКА ЗАПУСК/СТОП
        ...
        # Статус — ПРОСТОЙ LABEL БЕЗ РАМКИ И ФОНА
    ...
    def check_and_ask(self):
        ...
        if acc and SiteAccessibleDialog(d,self).exec_() == QDialog.Rejected:
    ...
    def toggle_blockcheck(self):
        """Переключает между запуском и остановкой blockcheck"""
        if self.bc_running:
    ...
    def run_blockcheck(self):
        ...
        self.btn_save.setEnabled(False)
        self.term.clear()
        self.status.setText(f"⏳ Запущен для {d}...")
        self.bc_thread = BlockcheckThread(d, self.current_settings, pwd)
    ...
    def on_stopped(self):
        """Вызывается после остановки blockcheck"""
        self.bc_running = False
        # Возвращаем кнопку в режим ЗАПУСК
        self.btn_run_stop.setText("▶ Запустить blockcheck")
        self.btn_run_stop.setStyleSheet("QPushButton{background:#3498db;color:white;font-weight:bold;padding:10px}QPushButton:hover{background:#2980b9}QPushButton:disabled{background:#95a5a6}")
        self.btn_check.setEnabled(True)
        self.status.setText("⏹ Blockcheck остановлен")
    
    def on_done(self, ok, strats, sets):
        self.bc_running = False
        # Возвращаем кнопку в режим ЗАПУСК
        self.btn_run_stop.setText("▶ Запустить blockcheck")
        self.btn_run_stop.setStyleSheet("QPushButton{background:#3498db;color:white;font-weight:bold;padding:10px}QPushButton:hover{background:#2980b9}QPushButton:disabled{background:#95a5a6}")
        ...
        if ok:
            has_working = "working without bypass" in self.term.toPlainText().lower()
            self.btn_save.setEnabled(len(strats) > 0 or has_working)
            if has_working and not strats:
                self.status.setText(f"✅ {self.get_domain()} работает без обхода — можно сохранить как «белый»")
            else:
    ...
    def save(self):
        ...
        if not d:
            return
        if not self.current_strategies and "working without bypass" not in self.term.toPlainText().lower():
            return
...
class ZapretManager(QMainWindow):
    def __init__(self):
        ...
        self.init_ui()
        self.update_status()
        # Таймер авто-проверки статуса каждые 5 секунд
    ...
    def init_ui(self):
        ...
        # Кнопки растягиваются до индикатора
    ...
    def update_status(self):
        """Авто-проверка статуса zapret (вызывается каждые 5 секунд)"""
        try:
