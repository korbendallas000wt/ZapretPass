...
get_selected_strategy() {
    local pid=$(pgrep -f "nfqws" | head -n1)
    if [ -n "$pid" ] && [ -f "/proc/$pid/cmdline" ]; then
        cat "/proc/$pid/cmdline" | xargs -0 | sed 's|/opt/zapret/nfq/nfqws|nfqws|'
    elif [ -f "$STRATEGY_CACHE" ]; then
...
show_info() {
    ...
    if [ -f "$SYS_WHITELIST" ]; then
        ...
        if [ "$count" -gt 0 ]; then
            ...
            if [ "$count" -gt 20 ]; then
                echo -e "    ${GRAY}... и ещё $((count - 20)) доменов${NC}"
            fi
        else
    ...
    if [ -n "$strategy" ]; then
        ...
        # Дата применения (время изменения конфига)
        if [ -f "/opt/zapret/config" ]; then
            date=$(stat -c %y /opt/zapret/config | cut -d'.' -f1)
            [ -n "$date" ] && print_status "info" "Применено: $date"
        fi
    else
...
while true; do
    show_info
    # Ждем Enter максимум 10 секунд (автообновление).
    # Если скрипт запущен в фоне (без терминала), read моментально вернет ошибку,
    # и сработает sleep 10, чтобы не жарить процессор.
    read -r -t 10 || sleep 10
done
