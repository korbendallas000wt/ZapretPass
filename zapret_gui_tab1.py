...
class SnifferThread(QThread):
    ...
    def stop(self):
        ...
        if self.process:
            try:
                # Убиваем ВЕСЬ пайплайн (bash + tshark + timeout + grep + awk)
    ...
    def run(self):
        try:
            ...
            # start_new_session=True создаёт отдельную группу процессов для корректного killpg
...
class BlockcheckTab(QWidget):
    ...
    def run_sniffer(self):
        ...
        # 🚀 Авто-старт zapret (если не активен)
        active, _ = get_zapret_status()
        if not active:
            ok, out, err = run_sudo(["sudo", "systemctl", "start", "zapret"], pwd)
            if not ok:
                self.term.appendPlainText(f"⚠️ Не удалось запустить zapret: {err}")
            # Если ok — молча продолжаем, если ошибка — не блокируем сниффер
    ...
    def stop_sniffer(self):
        if self.sniffer_thread and self.sniffer_thread.isRunning():
            self.sniffer_thread.stop()          # Останавливаем поток
            self.sniffer_update_timer.stop()    # Останавливаем таймер
            self.close_browser()                # Закрываем браузер

            # ⚡ НЕМЕДЛЕННО разблокируем интерфейс
            self.sniffer_running = False
            self.lock_ui(False)
            self.btn_sniffer.show()
            self.btn_sniffer_stop.hide()
            self.status.setText("⏹ Сниффер остановлен вручную")
            self.term.appendPlainText("\n⏹ Сниффер остановлен. Файл сохранён.")
